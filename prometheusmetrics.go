...
func (c *PrometheusConfig) gaugeFromNameAndValue(name string, val float64, extraLabels prometheus.Labels) {
	...
	*/
	...
	g, ok := c.gauges[key]
	if !ok {
		g = prometheus.NewGaugeVec(prometheus.GaugeOpts{ // Tapjoy - use GaugeVec instead of Gauge
			Namespace: c.flattenKey(c.namespace),
			Subsystem: c.flattenKey(c.subsystem),
			Name:      c.flattenKey(name),
			Help:      name,
		}, labelNames) // Tapjoy - add labels to gauge
	...
	g.With(labels).Set(val) //Tapjoy - set the val with labels
	/** ----- End Tapjoy Changes ----- */
...
func (c *PrometheusConfig) histogramFromNameAndMetric(name string, goMetric interface{}, buckets []float64, extraLabels prometheus.Labels) {
	...
	*/
	...
	desc := prometheus.NewDesc(
		...
		c.flattenKey(name),
		labels, // Tapjoy - add labels to histogram
		map[string]string{},
	)
	...
	if constHistogram, err := prometheus.NewConstHistogram(
		...
		sum,
		bucketVals,
		labelVals..., // Tapjoy - add labels to histogram
	); err == nil {
		c.mutex.Lock()
...
func (c *PrometheusConfig) UpdatePrometheusMetricsOnce() error {
	c.Registry.Each(func(name string, i interface{}) {
		...
		*/
		...
		/** ----- Tapjoy Changes -----
		Next, we're passing through the list of labels and also recording metrics
